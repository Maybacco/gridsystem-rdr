function ParseMarker(m, invoker)
    ...
    if not m.control or (type(m.control) ~= "string" and type(m.control) ~= "number") then m.control = Keys["G"]
    elseif type(m.control) == "string" then
        m.control = Keys[m.control] or Keys["G"]
    end
